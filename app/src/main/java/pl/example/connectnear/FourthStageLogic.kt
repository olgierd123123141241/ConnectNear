...
class SettingsState {
    ...
    fun applyToUserSelection(userSelection: UserSelection) {
        ...
        userSelection.dateAnimalType = tempDateAnimalType
        userSelection.dateOtherAnimal = tempDateOtherAnimal
...
class MapState(
    ...
    var flashEvents by mutableStateOf<List<FlashEvent>>(emptyList()) // Nowe
    var selectedUser by mutableStateOf<FoundUser?>(null)
    var hasPermission by mutableStateOf(false)
    var showSettingsDialog by mutableStateOf(false)

    var userCategorySnapshot by mutableStateOf(userSelection.category)
    ...
    init {
        if (userSelection.userId.isNotEmpty()) {
            FirebaseService.listenForNotifications(userSelection.userId) { data ->
    ...
    fun createFlashEvent(description: String, durationMs: Long, imageUrl: String? = null, isOneTime: Boolean = false) {
        if (myLocation != null) {
            val event = FlashEvent(
                id = "", // Firebase wygeneruje
                creatorId = userSelection.userId,
                category = userSelection.category, // Użyj kategorii użytkownika
...
fun rememberMapState(
    ...
    val permissionLauncher = rememberLauncherForActivityResult(
        ...
        onResult = { /* Logika uprawnień */ }
    )

    val mapState = remember(userSelection, snackbarHostState, scope) {
        MapState(userSelection, scope, snackbarHostState, permissionLauncher)
    }
    ...
    val s = mapState.settingsState
    LaunchedEffect(
        s.isSharingLocation, s.tempCategory, s.tempVisibilityMode, 
        s.tempInterval, s.tempRadius, s.tempPreferredAge, s.tempPreferredSex,
        s.tempSportMode, s.tempSportLevel, s.tempSubCategory, s.tempLearningMode, s.tempSubject, s.tempLearningLevel,
        s.tempDateMode, s.tempDatePartnerGender, s.tempDateCoupleGender, s.tempDateAnimalType, s.tempDateOtherAnimal,
        // NOWE
        s.tempPartyType
    ) {
        if (mapState.showSettingsDialog) {
            delay(1500) // Czekamy 1.5 sekundy
            mapState.saveSettings()
        }
    }

    LaunchedEffect(userSelection.locationUpdateInterval, mapState.hasPermission, userSelection.shareLocation, userSelection.category) { 
        if (mapState.hasPermission) {
            while (isActive) {
                try {
                    fusedLocationClient.lastLocation.addOnSuccessListener { location ->
                        if (location != null) {
                            ...
                            FirebaseService.getBlockedUsers(userSelection.userId) { blockList ->
                                FirebaseService.findMatches(userSelection) { users -> 
                                    mapState.otherUsers = users.filter { it.userId !in blockList }
                                }
                                // Filtrujemy wydarzenia, które już zostały obejrzane i są jednorazowe
                                if (userSelection.location != null) {
                                    FirebaseService.getActiveFlashEvents(userSelection.location!!, userSelection.searchRadiusKm) { events ->
                                        mapState.flashEvents = events.filter { event ->
                                            val isBlocked = event.creatorId in blockList
                                            ...
                                            !isBlocked && isCategoryMatch && !isViewedAndOneTime
                                        }
                                    }
                                }
                            }
