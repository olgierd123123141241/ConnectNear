...
object FirebaseService {
    ...
    fun saveCurrentUser(userSelection: UserSelection, onSuccess: () -> Unit) =
        ProfileRepo.saveCurrentUser(userSelection, onSuccess)
    ...
    fun findMatches(mySelection: UserSelection, onResult: (List<FoundUser>) -> Unit) {
        BlockedUserRepo.getBlockedUsers(mySelection.userId) { blockedUsers ->
            MapRepo.findMatches(mySelection) { foundUsers ->
                onResult(foundUsers.filter { !blockedUsers.contains(it.userId) })
            }
        }
    }
